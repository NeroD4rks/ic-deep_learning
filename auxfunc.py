...
def log_debug(txt: str, path: str= "file_log.log", printar=True):
  logging.basicConfig(filename=str(Path(path)), level=logging.INFO, format='%(asctime)s %(message)s', datefmt='%d/%m/%Y %I:%M:%S %p')
...
def preprocess(dataset, shape):
    ...
    """
    return np.array([tf.image.resize(img, size=[shape[0], shape[1]], method=tf.image.ResizeMethod.BICUBIC) for img in dataset])/255
...
def get_test_train(path: str, shape):
  """"
...
def augment_images_save(path: str, number: int, number_files : int=1):
  """
  ...
  for classe in classes:
    ...
    for _ in range(number_files):
      gen = ImageDataGenerator(rotation_range=10, width_shift_range=0.1, height_shift_range=0.1, shear_range=0.15, zoom_range=0.1, horizontal_flip=True) # definindo as ações que serão realizadas, exemplo, rotacionar
      chosen_image = random.choice(list(set(os.listdir(str(Path(f"{path}/{classe}/")))) - set(files)) ) # Para não repetir o que já foi usado
      if not chosen_image:
        break
      files.append(chosen_image)
      image_path = str(Path(f"{path}/{classe}/{chosen_image}")) # selecionando uma imagem aleatoria
      image = np.expand_dims(plt.imread(image_path),0)
      aug_iter = gen.flow(image, save_to_dir=f"{path}/{classe}/", save_prefix='aug-image-', save_format='png') # definindo pasta e prefixo (para deletar depois)
      log_debug(f"Gerando imagens a partir do arquivo {chosen_image} da classe {classe}")
      aug_images = [next(aug_iter)[0].astype(np.uint8) for i in range(number)] # realizando efetivamente o data augmentation, gerando conforme o número informado
      log_debug("Terminando de gerar imagens\n\n")

def del_augment_images(path: str):
  """
  ...
  for classe in classes:
    ...
    files = [file for file in os.listdir(Path(f"{path}/{classe}")) if "aug-image-" in file] # selecionando para deletar se tiver o prefixo
    for file in files:
